...
def find_measFiles(path, fileString, beam, freq_set, it):
    ...
    for i in range(len(files)):
        if fileString in files[i] and 'eam' + str(beam) in files[i] and f'{freq_set}_GHz_4' in files[i] and \
                f'teration_{it}' in files[i]:
...
def plot_tlm_map(array_in, title, cmin, cmax, cstp, f_set, plot_no, tick_step, delta_pol, TLM_Type, axs,
                 meas_frequencies, align=False, col_map='jet'):

    # map tlm as a df (and roated if needed)
    if TLM_Type == 'Rx':
        map_tlm_df = pd.read_csv(r'.\20240227_tlm_map_plotter\20221019_TLMCalInputs\/' +
                                 r'Mrk1_S2000_TLM_RX_ArrayGeometry_V20062022_CalInfo_Orig.csv', header=1)
    elif TLM_Type == 'Tx':
        map_tlm_df = pd.read_csv(r'.\20240227_tlm_map_plotter\20221019_TLMCalInputs\/' +
                                 r'Mrk1_S2000_TLM_TX_ArrayGeometry_V20062022_CalInfo.csv', header=1)
    ...
    # plot rfics
    if TLM_Type == 'Rx':
        map_rfic = pd.read_csv(r'.\20240227_tlm_map_plotter\20221019_TLMCalInputs\/' +
                               r'MK1_RX_TLM_RFIC_Patch_Feed_mapping_RF.csv')
    elif TLM_Type == 'Tx':
        map_rfic = pd.read_csv(r'.\20240227_tlm_map_plotter\20221019_TLMCalInputs\/' +
                               r'MK1_TX_TLM_RFIC_Patch_Feed_Mapping.csv')
    ...
    # print(v)
    cntr = axs[plot_no].scatter(map_tlm_df[' Feed x [mm]'], map_tlm_df[' Feed y [mm]'], c=Z_trim, marker=m, s=200,
                                edgecolors='black', linewidths=0.5, cmap=cmap_chosen, vmin=min(v), vmax=max(v),
                                alpha=1.0)
    ...
    if delta_pol is True:
        ...
        cntr = axs[plot_no].scatter(map_tlm_df[' Feed x [mm]'], map_tlm_df[' Feed y [mm]'],
                                    c=(Z_trim_pol1 - Z_trim_pol2), marker=m, s=200, edgecolors='black',
