"""
Plots the port maps with their gain and phase given the input data
created on 04/07/2024
author: Harry William Morris
"""
...
# set-up cal files and input data
file_path = r'.\2024-07-01_17-16-37_MCR1_Rig1_cal_QR00169_1_20_3820_Bias_0_Override_7_29.50_45C'
if TLM_Type == 'Rx':
    map_tlm_df = pd.read_csv(
        r'.\20240227_tlm_map_plotter\20221019_TLMCalInputs\Mrk1_S2000_TLM_RX_ArrayGeometry_V20062022_CalInfo_Orig.csv',
        header=1)
    freq_list = ['19.20']
elif TLM_Type == 'Tx':
    map_tlm_df = pd.read_csv(
        r'.\20240227_tlm_map_plotter\20221019_TLMCalInputs\Mrk1_S2000_TLM_TX_ArrayGeometry_V20062022_CalInfo.csv',
        header=1)
        ...
        for f_type in ['OP_2', 'RFA']:
            ...
            for beam in beam_list:
                ...
                for freq_set in freq_list:
                    ...
                    (meas_info, meas_params, meas_array, meas_frequencies, meas_array_gain, meas_array_phase, paramName,
                     i) = load__measFiles(measFiles[0])
                    ...
                    if 'OP' in f_type:
                        Z, Z_trim, map_tlm_df = plot_tlm_map(meas_array_av,
                                                             ...
                                                             f'\n Freq = {freq_set} GHz, Beam {beam}, Delta Pol='
                                                             f'{delta_pol}, Lens Align={align}',
                                                             -v_OP, v_OP, v_OP_step, float(meas_params['f_c']), 0,
                                                             tick_step=tick_step,
                                                             delta_pol=delta_pol, TLM_Type=TLM_Type, axs=axs,
                                                             meas_frequencies=meas_frequencies, align=align)
                    elif 'RFA' in f_type:
                        Z, Z_trim, map_tlm_df = plot_tlm_map(meas_array_av,
                                                             f'({f_type}) {gain_phase} (average), N = {len(measFiles)} '
                                                             f'\n Freq = {freq_set} GHz, Beam {beam}, Delta Pol='
                                                             f'{delta_pol}, Lens Align={align}',
                                                             -v_RFA, v_RFA, v_RFA_step, float(meas_params['f_c']), 0,
                                                             tick_step=tick_step,
                                                             delta_pol=delta_pol, TLM_Type=TLM_Type, axs=axs,
                                                             meas_frequencies=meas_frequencies, align=align)
                    ...
                    # plot the stdev todo change x axis from ports to RFIC from data
                    Z, Z_trim, map_tlm_df = plot_tlm_map(meas_array_std,
                                                         f'({f_type}) {gain_phase} (stdev), N = {len(measFiles)} '
                                                         f'\n Freq = {freq_set} GHz, Beam {beam}, Delta Pol={delta_pol}'
                                                         f', Lens Align={align}',
                                                         0.0, vmax_std, v_spread_step, float(meas_params['f_c']), 1,
                                                         tick_step=tick_step,
                                                         delta_pol=delta_pol, TLM_Type=TLM_Type, axs=axs,
                                                         meas_frequencies=meas_frequencies, align=align)
